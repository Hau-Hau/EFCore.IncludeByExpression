/// Slightly modified DotNext's SoftReference to match project needs
using System;
...
public sealed class SoftReference<T>
    where T : class
{
    ...
    public bool TryGetTarget([NotNullWhen(true)] out T? target) => (target = Target) is not null;
...
public class SoftReferenceOptions
{
    ...
    internal bool KeepTracking(object target)
    {
        ...
        return generation < GC.MaxGeneration
            || GC.CollectionCount(generation) <= collectionCount && CheckLimits(generation);
    }
